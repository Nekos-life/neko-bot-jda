...
public class RedTubeAudioSourceManager implements AudioSourceManager, HttpConfigurable {

    static final String CHARSET = "UTF-8";
    private static final String VIDEO_REGEX = "^https?://www.redtube.com/\\d{4,8}$";
    private static final Pattern VIDEO_INFO_REGEX = Pattern.compile("playervars: (\\{.+})");
    private static final Pattern videoUrlPattern = Pattern.compile(VIDEO_REGEX);
    private final HttpInterfaceManager httpInterfaceManager;

    public RedTubeAudioSourceManager() {
    ...
    public AudioTrack decodeTrack(AudioTrackInfo trackInfo, DataInput input) {
        return new RedTubeAudioTrack(trackInfo, this);
    }
    ...
    private AudioItem loadItemOnce(AudioReference reference) {
        try (HttpInterface httpInterface = getHttpInterface()) {
            ...
            int videoDuration =
                    Integer.parseInt(info.get("video_duration").text()) * 1000; // PH returns seconds

            return buildTrackObject(
                    reference.identifier, playbackURL, videoTitle, "Unknown Uploader", false, videoDuration);
        } catch (Exception e) {
            throw ExceptionTools.wrapUnfriendlyExceptions(
                    "Loading information for a RedTube track failed.", FAULT, e);
        }
    }
    ...
    public RedTubeAudioTrack buildTrackObject(
            String uri,
            String identifier,
            String title,
            String uploader,
            boolean isStream,
            long duration) {
        return new RedTubeAudioTrack(
                new AudioTrackInfo(title, uploader, duration, identifier, isStream, uri), this);
    }
}
