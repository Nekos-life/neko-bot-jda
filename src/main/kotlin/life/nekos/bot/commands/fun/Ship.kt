...
class Ship : Cog {
    ...
    suspend fun ship(ctx: Context, user1: User, user2: User = ctx.author) {
        if (user1.idLong == ctx.jda.selfUser.idLong) {
            return ctx.respondUnit("Nu nya, your tail's not big enough for me~ >.<")
        }

        if (user1.idLong == ctx.author.idLong && user2.idLong == ctx.author.idLong) {
            return ctx.respondUnit("Nu nya, you can't ship yourself~")
        }

        if (user1.idLong == user2.idLong) {
            return ctx.respondUnit("Nu nya, you must ship different people~")
        }

        ctx.asSlashContext?.deferAsync()
        ctx.asMessageContext?.messageChannel?.sendTyping()?.submit()?.await()
